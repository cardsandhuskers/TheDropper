...
public class SaveButtonLocationCommand implements CommandExecutor {
    private TheDropper plugin;

    public SaveButtonLocationCommand(TheDropper plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if(sender instanceof  Player p && p.isOp()) {
            if(args.length > 0) {
                ...
                plugin.getConfig().set("buttons." + level, l);
                plugin.saveConfig();
